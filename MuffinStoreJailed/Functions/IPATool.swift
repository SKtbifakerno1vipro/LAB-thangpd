...
class StoreClient {
    ...
    func authenticate(requestCode: Bool = false) -> Bool {
        ...
        // Skadz 7.25.26
        func attemptGetPod(url: URL, extraHeaders: [String: String] = [:], completion: @escaping (Bool, Data?, HTTPURLResponse?) -> Void) {
            var request = URLRequest(url: url)
...
//                print("requested from \(url.absoluteString)")
                ...
                if let pod = httpResponse.value(forHTTPHeaderField: "pod") {
                    ...
                    print("russia fix")
                    attemptGetPod(url: newURL, extraHeaders: extraHeaders, completion: completion)
                }
            }
            dataTask.resume()
        }
        ...
        Task {
            ...
            let anisetteHeaders = await getAnisetteHeaders()
            ...
            attemptGetPod(url: url, extraHeaders: anisetteHeaders) { success, data, response in
                if !success {
